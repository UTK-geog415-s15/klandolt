...
setwd("~/Documents/GEOG415/klandolt/homework2/")
library(ggplot2)
library(dplyr)
library(tidyr)
library(magrittr)
...
ggplot(temp.data, aes(month, precip)) + geom_bar(stat='identity') + labs(y='Precipitation (in)', x='Month')
```
...
ggplot(data=rwi.precip) + geom_point(aes(x=rwi, y=Jan))
```

```{r scat2 scatterplot for tree ring width and precipitation in August}
rwi.precip <- left_join(precip, rwi, by='year')
ggplot(data=rwi.precip) + geom_point(aes(x=rwi, y=Aug))
...
ggplot(data=totcor) + geom_point(aes(x=month, y=cor.r, color='current year')) + geom_point(aes(x=month, y=corlag.r, color='lagged year')) + labs(color='Correlation of') + geom_abline(intercept=0, slope=0)
```
